---
title: "Code to reproduce the analyses of the AWST paper: Analysis of the CBMC data"
...
if(!file.exists("expression.RData")) {
  ...
  library(awst)
  dim(exprData <- awst(normCounts, poscount = TRUE, full_quantile = TRUE)) #[1]  7613 17014
  dim(exprData <- gene_filter(exprData, nBins = 30, shannon_cutoff = 0.1)) #[1] 7613  205
...
if(save_plots) {
  mmain <- ""
  png(paste0(jobName, "_main_dendrogram.png"), width= png_width_large, height= png_height_large, res = png_res)
}
...
legend(1470, .95, legend=llegend, fill = ffill, y.intersp = 1, box.col = "white", border = "white", title = "CBMC", title.adj = 0)
