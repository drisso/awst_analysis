...
####
results <- matrix(NA, ncol = 4, nrow = 40)
colnames(results) <- c("where", "what", "silhouette", "R2")
k <- 1
save_plots <- FALSE
...
methods <- list("raw"=t(log1p(as.matrix(raw))), "cpm"=t(log1p(cpm)),
                "cpm2500"=t(log1p(cpm2500)), "fq"=t(log1p(fq)),
                "awst"=exprData, "awst_gf"=exprData_gf, "vst"=t(vst),
                "vst2500"=t(vst2500), "rlog"=t(rld), "rlog2500"=t(rld2500),
                "deviance"=t(res_deviance), "pearson"=t(res_pearson))

results[k:(k+7),1] <- names(methods)
results[k:(k+7),2] <- rep(prefix, length(methods))
results[k:(k+7),3] <- sapply(seq_along(methods), function(i) {
...
r2s <- lapply(seq_along(methods), function(i) {
  get_r2(methods[[i]], as.factor(annotation.df$cell.col))
})
names(r2s) <- names(methods)

# prendere solo i geni DE

results[k:(k+7), 4] <- sapply(r2s, mean, na.rm=TRUE)
