...
####
results <- matrix(NA, ncol = 5, nrow = 90)
...
methods <- list("raw"=t(log1p(as.matrix(raw))), "cpm"=t(log1p(cpm)),
                ...
                "vst5000"=t(vst5000), "rlog"=t(rld), "rlog1000"=t(rld1000),
                "rlog2500"=t(rld2500), "rlog5000"=t(rld5000),
                "deviance"=t(res_deviance), "pearson"=t(res_pearson))

results[k:(k+17),1] <- names(methods)
results[k:(k+17),2] <- rep(prefix, length(methods))
results[k:(k+17),3] <- sapply(seq_along(methods), function(i) {
...
results[k:(k+17), 4] <- sapply(de_genes, NCOL)
...
results[k:(k+17), 5] <- sapply(r2s, mean, na.rm=TRUE)
